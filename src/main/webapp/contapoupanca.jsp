...
<title>Cadastro Conta Poupanca</title>
</head>
<body>
	<br />
			...
			<table>
				...
				<tr>
					<td colspan="4">
						...
						value='<c:out value="${contaPoupanca.dataAbertura}"/>'
						class="input-group input-group-lg">
					</td>
				</tr>		
				<tr>
					<td colspan="4">
						...
						value='<c:out value="${contaPoupanca.saldo}"/>'
						class="input-group input-group-lg">
					</td>
				</tr>
				<tr>
					<td colspan="3">
						<input type="number"
						id="percentual_rendimento" name="percentual_rendimento" step="0.01" placeholder="Percentual Rendimento"
						value='<c:out value="${contaPoupanca.percentualRendimento}"/>'
						class="input-group input-group-lg" >
					</td>
				</tr>
				<tr>
					<td colspan="4">
						<input type="date" 
						id="dia_abertura" name="dia_abertura" placeholder="Dia de Aniversário"
						value='<c:out value="${contaPoupanca.diaAniversario}"/>'
						class="input-group input-group-lg">
					</td>
				</tr>	
				<tr>
			...
			<table class="table table-dark table-striped">
				<thead>
					<tr>
						...
						<th>Data Abertura</th>
						<th>Saldo</th>
						<th>Percentual Rendimento</th>
						<th>Dia Aniversário</th>
						<th></th>
						<th></th>
					</tr>
				</thead>
				<tbody>
					<c:forEach var="c" items="${contas_poupancas}">
						<tr>
							<td>${c.codigo}</td>
							<td>${c.dataAbertura}</td>
							<td>${c.saldo}</td>
							<td>${c.percentualRendimento}</td>
							<td>${c.diaAniversario}</td>
